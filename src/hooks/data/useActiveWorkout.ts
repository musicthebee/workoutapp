...
import type { ActiveExercise, ActiveSetPerformance, UUID } from '@/types';
...
export const useActiveWorkout = () => {
  ...
  const can_add_exercise = session
    ? ['preparing', 'active', 'rest'].includes(session.state)
    : false;

  // Enhanced log set with validation
  const log_set_performance = useCallback(
    (performance: ActiveSetPerformance) => {
      if (!current_exercise) {
  ...
  const finish_workout = useCallback(
    async (notes?: string) => {
...
export const useSetLogger = (exercise: ActiveExercise | null) => {
  ...
  const quick_reps = useMemo(() => {
    if (!exercise?.currentSet?.targetReps) {
      return [];
    }
    const target = exercise.currentSet.targetReps;
    return [target - 2, target - 1, target, target + 1, target + 2].filter(r => r > 0);
  }, [exercise]);
