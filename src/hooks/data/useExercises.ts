...
export const useExercises = () => {
  ...
  const create_new_exercise = useCallback(
  ...
  const update_existing_exercise = useCallback(
    async (id: UUID, updates: UpdateExerciseInput) => {
      try {
  ...
  const copy_library_exercise = useCallback(
    async (library_exercise_id: UUID) => {
      try {
  ...
  // Batch operations
  const archive_multiple = useCallback(
    async (exercise_ids: UUID[]) => {
      const results = await Promise.allSettled(exercise_ids.map(id => archive_exercise(id)));
...
export const useExercise = (exercise_id: UUID | null | undefined) => {
  ...
  const { update_exercise, toggle_favorite, archive_exercise } = useExerciseStore(state => ({
  ...
  const update = useCallback(
    async (updates: UpdateExerciseInput) => {
...
export const useExerciseCreation = () => {
  ...
  const create_manual = useCallback(
    async (input: Omit<CreateExerciseInput, 'user_id'>) => {
