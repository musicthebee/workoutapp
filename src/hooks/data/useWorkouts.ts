...
export const useWorkouts = () => {
  ...
  // Filter handlers
  const handle_filter_change = useCallback(<K extends keyof WorkoutFilters>(
    key: K,
    value: WorkoutFilters[K]
  ) => {
  ...
  // CRUD operations
  const create_new_workout = useCallback(async (input: CreateWorkoutInput) => {
    try {
  ...
  const update_existing_workout = useCallback(async (
    id: UUID,
  ...
  const copy_library_workout = useCallback(async (library_workout_id: UUID) => {
    try {
...
export const useWorkout = (workout_id: UUID | null | undefined) => {
  ...
  const exercises_with_details = useMemo(() => {
    return workout_exercises.map(we => {
  ...
  const update = useCallback(async (updates: UpdateWorkoutInput) => {
